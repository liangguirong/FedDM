...
def consistency_loss(dataset,logits_s, logits_w,time_p,p_model, name='ce',threshold=None,avg_local_label=None, use_hard_labels=True):
    ...
    if name == 'L2':
        ...
        local_label = (p_model / sum(p_model))
        threshold = avg_local_label + p_cutoff.cpu() - avg_local_label.std()
        threshold[threshold >= 1] = 1
        # threshold = p_cutoff * p_model_cutoff[max_idx]
        mask = max_probs.ge(threshold[max_idx].cuda())
        if use_hard_labels:
        ...
        return masked_loss.mean(), max_idx[mask], threshold,mask
