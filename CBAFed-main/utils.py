...
def synthesize(args,train_dl):
    ...
    net_glob = get_network(args.model, channel, num_classes, im_size) # get a random model
    state = torch.load(os.path.join(args.datadir,'net_glob.pth'))
    w = state['state_dict']
    net_glob.load_state_dict(w)
    net_glob.cuda().eval()
    total = 0
    idx = 0
    for i, (_, weak_image_batch, label_batch) in enumerate(train_dl):
        # obtain pseudo labels
        with torch.no_grad():
            image_batch = weak_image_batch[0]
            total = total + len(image_batch)
            image_batch = image_batch.cuda()
            outputs = net_glob(image_batch)
            guessed = F.softmax(outputs, dim=1).cpu()
            pseu = torch.argmax(guessed, dim=1).cpu()
            for image_b in image_batch:
                images_all.append(torch.unsqueeze(image_b, dim=0))
            for lab in pseu:
                indices_class[lab.long()].append(idx)
                idx = idx + 1
    images_all = torch.cat(images_all, dim=0).to("cpu")
    for c in range(num_classes):
        print('class c = %d: %d real generate images'%(c, len(indices_class[c])))
...
def datadistill(args,model,train_dl,class_confident, dsa):
    args.device = 'cuda' if torch.cuda.is_available() else 'cpu'
    num_classes = args.n_classes
    def get_images(c, n):  # get random n images from class c
        idx_shuffle = np.random.permutation(indices_class[c])[:n]
    ...
    for i, (_, weak_image_batch, label_batch) in enumerate(train_dl):
        ...
        with torch.no_grad():
            ...
            for image_b in select_samples:
                images_all.append(torch.unsqueeze(image_b, dim=0))
            for lab in pl:
                label_all.append(lab.item())
                indices_class[lab.long()].append(idx)
                idx = idx + 1
    images_all = torch.cat(images_all, dim=0).to("cpu")
    label_all = torch.tensor(label_all)
    image_syn = images_all[0:num_classes * args.ipc]
    label_syn = label_all[0:num_classes * args.ipc]
    for c in range(num_classes):
        print('class c = %d: %d real generate images' % (c, len(indices_class[c])))
    ''' initialize the synthetic data '''
    for c in range(num_classes):
        if len(indices_class[c]) >= args.ipc:
            image_syn.data[c * args.ipc:(c + 1) * args.ipc] = get_images(c, args.ipc).detach().data
            label_syn.data[c * args.ipc:(c + 1) * args.ipc] = c
    distilled_dataset = TensorDataset(image_syn, label_syn)
    distilled_loader = torch.utils.data.DataLoader(distilled_dataset, batch_size=64, shuffle=True)
    model.train()
    model.cuda()
    ...
    for _ in range(distill_iter):
      for (_, x_dis, y_dis) in distilled_loader:
        x_dis , y_dis = x_dis.to(args.device), y_dis.to(args.device)
        if dsa:
...
def epoch(mode, dataloader, net, optimizer, criterion, args, aug, texture=False):
    ...
    for i_batch, datum in enumerate(dataloader):
        img = datum[1].float().to(args.device)
        lab = datum[2].long().to(args.device)

        if mode == "test":
            lab = lab.long().squeeze().cuda()
        if aug:
            if args.dsa:
