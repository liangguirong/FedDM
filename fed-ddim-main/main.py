...
def parse_args_and_config():
    ...
    parser.add_argument("--generate", action="store_false")  #  new
    ...
    parser.add_argument("--n_classes", type=int, default=10, help="Random seed")
    parser.add_argument(
        "-i",
        "--image_folder",
        type=str,
    ...
    parser.add_argument(
        "--generate_sum", type=int, default=50000, help="number of generate"
    )
    parser.add_argument(
        "--eta",    # ？
        type=float,
...
def main():
    ...
    try:
        ...
        state = torch.load('../CBAFed-main/noniid_10%labeled.pth')
        args.generate_sum = state['X_train'].shape[0]
        if args.sample:
            runner.sample()
        elif args.test:
            runner.test()
            ...
            for i in range(n_clients):
                ...
                data = state['X_train'][state['net_dataidx_map'][i]]
                targets = state['y_train'][state['net_dataidx_map'][i]]
                ################# 获取每类样本的数量，并保存在文件中
