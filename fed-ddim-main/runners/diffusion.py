...
class Diffusion(object):
    ...
    def train(self, client , idx, r):
        ...
        # basicdataset, test_dataset = get_dataset(args, config)
    ...
    def sample(self):
        ...
        # 加
            ...
            self.sample_generate(model)    #  1w  Cifar10    1.2w FashionMNIST
        else:
            raise NotImplementedError("Sample procedeure not defined")

    # 0,1 已生成
    def sample_generate(self,model):
        '''
        num : 总样本的数量
        '''
        config = self.config
        num = self.args.generate_sum*2
        ...
        for r in range(n_round):
            ...
            for j in range(x[-1].size(0)):
                tvu.save_image(
                    x[-1][j], os.path.join(self.args.image_folder, f"{0}_{temp}.png")
                )
                temp += 1
        dur = time.time() - data_start
        print("生成完毕，耗时{}!!!!!！".format(dur))
